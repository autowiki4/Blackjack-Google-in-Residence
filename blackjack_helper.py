...
#   user: contains the user's name
#
# Return:
#   none
def print_end_game_status(user_hand, dealer_hand, user):
  # customize the print for the game result to return user or push

  if user_hand <= 21 and (user_hand > dealer_hand or dealer_hand > 21):
    print(f'{user} wins!', end='')
    return user
  elif user_hand > 21 or (dealer_hand <= 21 and dealer_hand > user_hand):
    print(f'{user} loses!', end='')
  else:
    print(f'{user} pushes.', end='')
    return 'push'